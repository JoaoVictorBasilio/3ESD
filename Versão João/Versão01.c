/*----------------------------------------------------------------------
                        SISTEMA DE AFINIDADES - V1

   Dados armazenados em estruturas paralelas:
   - nomes[i] guarda o nome da pessoa i;
   - notas[i][j] guarda a nota da pessoa i para a preferencia j.
----------------------------------------------------------------------*/
...
float leNotaValida();
void cadastrarPessoas(char nomes[][50], float notas[][QTD_PREF], int quantidade);
void exibirPessoas(char nomes[][50], float notas[][QTD_PREF], int quantidade);
int buscaPessoa(char nomes[][50], int quantidade, char pessoa[]);
void encontrarPessoa(char nomes[][50], float notas[][QTD_PREF], int quantidade);
            ...
            switch (opcao)
            {
                case 1:
                    do
                    {
                        ...
                        if (quantidade < 1 || quantidade > MAX_PESSOAS)
                        {
                            printf("Quantidade invalida. Digite entre 1 e %d.\n",
                                   MAX_PESSOAS);
                        }
                    }
                    while (quantidade < 1 || quantidade > MAX_PESSOAS);
