...
)
{
    ...
    Pessoa **ranking; /* Vetor cujos elementos sao ponteiros Pessoa *. */
    float *distancias;
    ...
    ranking = malloc((quantidade - 1) * sizeof(Pessoa *));
    if (ranking == NULL)
    {
        printf("Erro ao alocar memoria para o ranking.\n");
        return;
    }
    ...
    for (i = 0; i < quantidade; i++)
    {
        if (i != indice)
        {
            /* & obtem o endereco da pessoa, sem copiar sua struct. */
            ranking[quantidadeRanking] = &pessoas[i];
    ...
    for (i = 0; i < quantidadeRanking - 1; i++)
    {
        for (j = i + 1; j < quantidadeRanking; j++)
        {
            if (distancias[j] < distancias[i])
            {
                ...
                /* Troca os enderecos, preservando o cadastro original. */
                pessoaAuxiliar = ranking[i];
                ranking[i] = ranking[j];
                ranking[j] = pessoaAuxiliar;
            }
        }
    }
...
int main()
{
    ...
    do
    {
        ...
        switch (opcao)
        {
            case 1:
                ...
                /*
                 * Se ja existia um cadastro,
                 * libera a memoria anterior.
                 */
                if (pessoas != NULL)
                {
                    free(pessoas);
    ...
    /*
     * Libera a memoria alocada para as pessoas.
     */
    free(pessoas);
